2022-0113-02)NULL처리함수
    - NULL은 길이가 없는 자료
    - 연산에 NULL이 사용되면 모든결과는 NULL이다.
    - NULL 관련 함수는 NVL, NVL2, NULLIF와 데이터가 NULL과 같은지 판단을 위해
      IS NULL, IS NOT NULL, 두 연산자가 제공됨 ( = 과 같은 비교연산자 사용불가능 )
       
    1.  IS NULL, IS NOT NULL
        .NULL값과 동성 평가를 위한 연산자
        .NULL은 '='연산자로 판별 불가능
...
2. NULL 처리함수
   ...
   - 'col'과 'val'은 반드시 같은 타입이어야 함 --(숫자,숫자) o / (숫자/글자) x
   
사용예) 2005년 6월 모든 상품에 대한 상품별 매입현황을 조회
        Alias는 상품별, 매입수량집계, 매입금액집계 --외부조인(수식어구로 모든이 붙는다)
