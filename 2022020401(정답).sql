...
사용예)상품코드를 입력받아 2005년 매출수량과 매출금액 및
  ...
  IS
    ...
    V_AMT NUMBER:=0;
  BEGIN
    SELECT A.PROD_NAME, SUM(B.CART_QTY), 
           SUM(B.CART_QTY*A.PROD_PRICE)
...
사용예)부서번호를 입력받아 부서명,인원수,주소를 반환하는
      프로시져 작성
...
사용예) 년도와 월을 입력 받아 해당 월에 가장 많이 구매한 회원의
       회원번호,이름,주소,마일리지를 반환하는 프로시져 작성
